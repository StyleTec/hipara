...
#include <libyara\include\yara\globals.h>
#include <libyara\include\yara\utils.h>
#include <libyara\include\yara\mem.h>
#include <libyara\include\yara\re.h>
#include <libyara\include\yara\error.h>
#include <libyara\include\yara\re_lexer.h>
#include <libyara\include\yara\hex_lexer.h>
    ...
    RE* re)
{
  ...
  {
    string->c_string[string->length++] = (char)node->right->value;
    node = node->left;
  }

  string->c_string[string->length++] = (char)node->value;
    ...
    while(fiber != NULL)
    {
      ...
      switch(*ip)
      {
        ...
        case RE_OPCODE_MATCH_AT_END:
          action = ((int)input_size > count) ? ACTION_KILL : ACTION_CONTINUE;
          break;
